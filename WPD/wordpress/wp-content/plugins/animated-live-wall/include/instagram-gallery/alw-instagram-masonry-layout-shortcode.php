		...
		if ( isset( $instagram_data['data'] ) ) {
			...
			foreach ( $instagram_data['data'] as $attachment_id ) {
				...
				if ( $alw_grid_thumb_size == 'thumbnail' ) {
					$thumbnail_url = $attachment_id['images']['thumbnail']['url']; }
				if ( $alw_grid_thumb_size == 'medium' ) {
					$thumbnail_url = $attachment_id['images']['low_resolution']['url']; }
				if ( $alw_grid_thumb_size == 'large' ) {
					$thumbnail_url = $attachment_id['images']['standard_resolution']['url']; }
				if ( $alw_grid_thumb_size == 'full' ) {
					$thumbnail_url = $attachment_id['images']['standard_resolution']['url']; }
				?>
				<div class="brick">
					<a class='grid' target='<?php echo esc_attr( $alw_insta_link ); ?>' href='<?php echo esc_url( $insta_photos_link ); ?>'>
						<div class='snip1467'><img src='<?php echo esc_url( $thumbnail_url ); ?>' width='100%'>
							...
							if ( $alw_insta_icon == 'instagram' ) {
								...
								?>
								<i class="pw-heart far fa-heart"> <?php echo esc_attr( $insta_photos_like ); ?></i>
								<i class="pw-comment far fa-comment"> <?php echo esc_attr( $insta_photos_comment ); ?></i>
