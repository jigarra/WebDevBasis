...
if (isset($_POST['bookname']) && isset($_POST['authorname'])) {
    ...
    if (!empty($bookname) && !empty($authorname) && !empty($discription)) {
        ...
        if ($uploadStatus == 1) {
            ...
            $uploadbook = "SELECT * from add_book WHERE bookname = '$bookname' AND addition = '$addition'";
            $result = $db->query($uploadbook);
            if ($result->num_rows >= 1) {
                $response['status'] = 0;
                $response['message'] = 'Woops..! The book is Already added..!';
            } else {
                $sqlQ = "INSERT INTO add_book (bookname, auther, discription, image, addition) VALUES (?,?,?,?,?)";
